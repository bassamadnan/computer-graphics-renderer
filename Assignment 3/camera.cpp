...
Ray Camera::generateRay(int x, int y) {
    float rand1 = next_float(), rand2 = next_float();
    float new_x = x + rand1, new_y = y + rand2;
    Vector3f pixelCenter = this->upperLeft + rand1 * (this->pixelDeltaU) + rand2*(this->pixelDeltaV);

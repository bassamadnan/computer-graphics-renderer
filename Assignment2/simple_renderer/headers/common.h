...
struct Tri {
    ...
    Tri() {}
    Tri(Vector3f& v1, Vector3f& v2, Vector3f& v3) : v1(v1), v2(v2), v3(v3) {}

    double Area(){
        Vector3f cp = Cross(v1 - v3, v2 - v3);
        double mag = sqrt(Dot(cp, cp));
        double area = mag /2;
        return area;
    }
};
...
struct Interaction {
    ...
    Vector3f f = Vector3f(M_1_PI); //constant diffuse color
    Vector3f neighbor = Vector3f(M_1_PI), bilinear = Vector3f(M_1_PI);
};